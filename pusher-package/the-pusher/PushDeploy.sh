...
# The good stuff! This is where we decide on which extension to use
# and do the deployment accordingly
for server in "${!allAppServers[@]}"; do
    ...
    for application in ${allAppServers[$server]}; do
        ...
        if [ ! -z "$server" ]; then
            if [ "$uiContextPath" == "$currentContextPath" ]; then
                            log detail "Deploying UI..."
                            # Deploy HTML 5 assets
                            ( deployUI $server )
            elif [[ ( "integrator" == "$currentContextPath" ) || ( "cortex-aria" == "$currentContextPath" ) ]]; then
                log detail "Checking if Felix is deployed..."
                ( deployFelixContainer $currentContextPath $server )
                log detail "Deploying $currentContextPath..."
                ( deployOsgiPackage $server "felix-${currentContextPath}" )
                ...
                log detail "Deploying $currentContextPath to $appServer..."
                ( deployWeblogicApp )
                if [[ ! ${uniqueWeblogicServers[*]} =~ "$server" ]]; then
                    uniqueWeblogicServers+=($server)
                fi
            fi
        fi
    done
...
# Check if we need to do an AEM package deploy to WebLogic or Tomcat
if [ "$installEPForAemPackage" == "true" ]; then
    log detail "Deploy AEM packages to AEM Quickstart Server ..."

	if [ ! -z $aemAuthorInstances ]; then
	    authCount=0
        for server in $aemAuthorInstances
        do
            author_pids=$(ssh ${aemAuthorLocalUsers[$authCount]}@$server "ps aux | grep '[a]uthor' | awk '{print \$2}'")
            if [ ${#author_pids[@]} -eq 0 ]; then
                log detail "The server at $server does not have an AEM Author Instance running"
            else
                ( deployAemToAppServer $server $aemAuthorPort ${aemAuthorLocalUsers[$authCount]} ${aemAuthorFolderPaths[$authCount]} $author_pids $aemAuthorContext )
            fi
	...
	if [ ! -z $aemPublishInstances ]; then
	    pubCount=0
        for server in $aemPublishInstances
        do
            publish_pids=$(ssh ${aemPublishLocalUsers[$pubCount]}@$server "ps aux | grep '[p]ublish' | awk '{print \$2}'")
            if [ ${#publish_pids[@]} -eq 0 ]; then
                log detail "The server at $server does not have an AEM Publish Instance running"
            else
                ( deployAemToAppServer $server $aemPublishPort ${aemPublishLocalUsers[$pubCount]} ${aemPublishFolderPaths[$pubCount]} $publish_pids $aemPublishContext )
            fi
